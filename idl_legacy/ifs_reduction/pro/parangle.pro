function parangle, ha, dec, latitude, DEGREE=degree, TCS=tcs
;+
; NAME:
;      PARANGLE
;
...
;; VLT TCS formula
;;
;; **  ha: hour angle in radians (geocentric apparent)
;; ** dec: declination in radians (geocentric apparent)
;; ** lat: observatory latitude in radians (geodetic)
if (keyword_set(tcs) eq 1) then begin
   sqsz = cos(d2r*latitude) * sin(d2r*had)
   cqsz = sin(d2r*latitude) * cos(d2r*dec) - $
          cos(d2r*latitude) * sin(d2r*dec) * cos(d2r*had)
   
   return,r2d*atan(sqsz,cqsz)
endif
