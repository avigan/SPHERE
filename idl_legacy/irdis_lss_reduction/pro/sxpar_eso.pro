function SXPAR_ESO, hdr, name, abort, COUNT=matches, COMMENT = comments, $
...
;
; CALLING SEQUENCE:
;      result = SXPAR_ESO( Hdr, Name, [ Abort, COUNT=, COMMENT =, /NoCONTINUE  ])   
;
; INPUTS:
...
;       A. Vigan, 2012, Modified to support long keywords specific to ESO data
;-
;----------------------------------------------------------------------
...
;  value of the number field in NUMBER.
   ...
   hie = where(keyword eq 'HIERARCH',chie)
   if (chie ne 0) then begin
      for i=0,chie-1 do begin
         idx = hie[i]
         key = (strsplit(hdr[idx],'=',/extract))[0]
         keyword[idx] = strtrim(key)
      endfor
   endif
...
; Process string parameter 
   ...
   if matches GT 0 then begin
      ...
      if eso then begin
         all = strsplit(line,'=',/extract)
         svalue = strtrim(strjoin(all[1:*],''),2)
      endif
