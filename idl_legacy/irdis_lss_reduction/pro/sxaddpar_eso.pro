Pro sxaddpar_eso, Header, Name, Value, Comment, Location, before=before, $
...
;  Make sure Name is 8 characters long
  ...
  eso = 0B
  short = strmid(strupcase(name),0,8)
  if (short ne 'HIERARCH') then begin
...
;  Extract first 8 characters of each line of header, and locate END line
  ...
  hie = where(keywrd eq 'HIERARCH',chie)
  if (chie ne 0) then begin
     for i=0,chie-1 do begin
        idx = hie[i]
        key = (strsplit(header[idx],'=',/extract))[0]
        keywrd[idx] = strtrim(key)
     endfor
  endif
  ...
  case type[1] of               ;which type?
     ...
     7:      begin
        ...
        if (upval eq 'T') or (upval eq 'F') then begin
           if eso then begin                 ;ESO keyword?
               strput,h,upval + $
                  ' /' + ncomment,strlen(name)+2
              header[i] = h
              return             
           endif
     ...
     5:      BEGIN
        ...
        if (eso) then begin     ;ESO keyword?
           strput,h,v + ' /' + ncomment,strlen(name)+2
           header[i] = h
           return           
        endif
